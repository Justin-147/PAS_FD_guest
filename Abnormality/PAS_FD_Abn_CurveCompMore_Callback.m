...
canshu=handles.canshu.canshu32;%读入配置文件参数
tmp=canshu{1};
tjlx=[];
...
for iiNFZ=1:1:NFZ
    ...
    PlotCurveComp(datam,dataz,timet,QS,wzFtimeCO,wzFtimeAM,outname,Range,flag);
    %输出
    if ismember(1,tjlx)
        saveas(gcf,outname,'tif');
    end
    if ismember(2,tjlx)
