% --------------------------------------------------------------------
%  PAS_FD_Plots_Single_Callback(hObject, eventdata, handles)
...
for iiNFZ=1:1:NFZ
    ...
    %%%%%%%%%%%%%%%%%%%%%%%
    FF=Fname{iiNFZ};
    if length(FF)>=12%文件名有可能是标准格式
        cx=strmatch(FF(9:12),SFLDM);%判断测项是否包含在内置表内
        if ~isempty(cx)%测项信息在内置表中
            if ylf==1%需要自动标注
                if ismember(cx,2:13)
                    yl='倾斜量/ms';
                elseif ismember(cx,14:21)
                    yl='应变量/10^{-10}';
                elseif cx==22
                    yl='应变量/10^{-9}';
                elseif cx==1
