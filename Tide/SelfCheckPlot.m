% --------------------------------------------------------------------
%  SelfCheckPlot(dep,alldbfile,allkkx,FWJ,TZM,FLDM,tjlx)
%  绘制曲线及自检关系（大于等于3测项才有效）
% --------------------------------------------------------------------
function SelfCheckPlot(dep,alldbfile,allkkx,FWJ,TZM,FLDM,tjlx)
tmpnn=alldbfile{1};
...
datetick('x','yyyymmdd');
set(gca,'Position',[0.1500 0.1750 0.7550 0.7250])
set(gca,'tickdir','out','FontName',FN,'FontSize',FS);
xlabel('日期','FontName',FNNL,'FontSize',FS);
ylabel('应变观测/10^{-10}','FontName',FNNL,'FontSize',FS);
title(tname,'FontName',FNNL,'FontSize',FS);
